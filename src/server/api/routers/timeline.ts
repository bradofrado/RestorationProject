...
const getCategoryDeprecated = (categoryName: TimelineCategoryName) => {
  ...
  if (category == undefined) {
    throw new TRPCError({
      code: 'BAD_REQUEST',
      message: `Invalid cateogry name ${categoryName}`,
    });
  }
...
export const timelineRouter = createTRPCRouter({
  getItems: publicProcedure.input(z.string()).query(async ({ ctx, input }) => {
    await ctx.logger.warn(`DEPRECATED getItems: ${input}`, ctx.session?.user);

    if (input) {
      const category = getCategoryDeprecated(input);
      return category.items;
    } else {
      const items = categories.reduce<RestorationTimelineItem[]>(
        (prev, curr) => {
