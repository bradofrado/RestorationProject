...
const EditItemsButtons = <T,>({
  ...
  return (
    <>
      <Dropdown items={items} onChange={onDropdownChange} initialValue={value}>
        select
      </Dropdown>
      <span className="mx-1">
        {value == undefined ? (
          <Button onClick={onAction(onAdd, true)} mode="secondary">
            Add
          </Button>
        ) : (
          <>
            <Button className="mr-1" onClick={onAction(onSave, false)}>
              Save
            </Button>
            {!isNew && (
              <ConfirmButton
                className="mr-1"
                onConfirm={onAction(onDelete, false)}
                mode="secondary"
            ...
            <Button onClick={onAction(onClear, false)} mode="secondary">
              Clear
            </Button>
          </>
        )}
