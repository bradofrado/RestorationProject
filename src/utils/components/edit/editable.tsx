...
export interface EditableDeleteableComponentProps<T>
...
export type ContentEditableComponent = {
...
function Editable<T extends React.ElementType>({
  ...
  return (
    <div className="relative">
      <div className="peer rounded-md hover:bg-sky-200/50">{render}</div>
      <div className="invisible absolute -top-8 z-20 opacity-0 transition-all hover:visible hover:opacity-100 peer-hover:visible peer-hover:opacity-100 peer-focus:visible peer-focus:opacity-100">
        {icons?.map((icon, i) => {
          ...
          if ('icon' in icon) {
            ...
            return (
              <button
                className={`rounded-md bg-slate-50 p-1 hover:bg-slate-300 ${className}`}
