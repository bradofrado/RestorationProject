...
type DraggableComponentProps = ReplaceWithName<
...
export const DroppableContext = <T,>({
  onReorder,
  id,
  children,
  items,
...
>;
export const DraggableListComponent = <T extends { id: number } | string>({
  children,
  items,
  ...rest
}: DraggableListComponentProps<T>) => {
  ...
  return (
    <>
      <DroppableContext items={items} {...rest}>
        {items.map((item, i) => (
          <DraggableComponent key={getId(item)} id={getId(item)} index={i}>
            {children(item, i)}
          </DraggableComponent>
        ))}
      </DroppableContext>
...
export const DirtyDraggableListComponent = <T extends { id: number }>({
  children,
  items: itemsProps,
  onReorder: onReorderProps,
  onReordering,
  ...rest
}: DirtyDraggableListComponentProps<T>) => {
  ...
  return (
    <>
      {isDirty && (
        <div>
          <Button className="mx-1" mode="primary" onClick={onSave}>
            Save
          </Button>
