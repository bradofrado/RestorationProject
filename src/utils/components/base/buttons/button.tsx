...
export type ButtonProps<C extends React.ElementType> =
  PolymorphicComponentProps<C, ButtonPropsInner>;
const Button = <T extends React.ElementType>({
  ...
  const buttonClasses: { [key in ButtonType]: string } = {
    primary:
      'bg-primary text-white hover:bg-opacity-80 focus-visible:outline-primary-light',
  ...
  const _class = `${className || ''} ${
    buttonClasses[mode]
  } inline-flex border justify-center rounded-md px-2 py-1 text-sm font-semibold leading-6 shadow-sm hover:bg-opacity-80 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2`;
  return (
    <>
